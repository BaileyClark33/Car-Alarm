...
-        If the ignition enabled (green LED) is lit, then normal ignition occurs. 
Light the blue LED and extinguish the green LED. Display the message, “Engine started.”
-        If the ignition is not enabled (green LED not lit), then ignition is inhibited. 
Sound the alarm buzzer; display the message, “Ignition inhibited” and display 
all the reasons why the ignition was inhibited: “Passenger seat not occupied,” 
“Driver seatbelt not fastened,” etc.
...
Keep the engine running even if the driver or passenger should remove their seat 
belts or exit the vehicle.
When the engine is running, stop the engine when the ignition button is pushed 
and then released. (note this change from Project 1 requirements)
...
if (headlightSelect == ON) {
  rightLamp == ON
  leftLamp == ON 
} else if (headlightSelect == OFF) {  // if they select OFF, turn off all lamps.
  rightLamp == OFF
  leftLamp == OFF
} else if (headlightSelect == AUTO) {  // If the user selects the AUTO position: 
  if (lightSensor >= daylightMin) { // When the light level is above a “daylight” level, 
    // switch headlights off after a delay of 2 seconds.
    delay(2000);
      rightLamp == OFF;
      leftLamp == OFF;
  } else if (lightSensor <= duskMax) {  // When the light level is below a “dusk” level, 
